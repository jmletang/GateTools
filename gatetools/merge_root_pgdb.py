        ...
        if not hists[hist]["rootDictValue"] == {} or not hists[hist]["rootDictType"] == {}:
            for branch in hists[hist]["rootDictValue"]:
                for i in range(len(hists[hist]["rootDictValue"][branch])):
                    if (("GammaZ;1" in branch) or ("EpEpgNorm;1" in branch) or ("SigmaInelastic;1" in branch)) :
                        hists[hist]["rootDictType"][branch][0][i] = hists[hist]["rootDictValue"][branch][i] / len(rootfiles)
                    else:
